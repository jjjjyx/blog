    ...
    <div class="post-writer-content curd-container">
        <div class="post-body-content" ref="postBody">
                    ...
                    <Poptip placement="bottom-start" width="200" transfer>
                        ...
                        <div class="post-tag-list" slot="content">
                            <CheckboxGroup v-model="selectedTag" @on-change="handleSelectTag">
                                <Checkbox :label="tag.name" v-for="(tag,index) in tagsList" :key="index">{{tag.name}}
                                </Checkbox>
                            </CheckboxGroup>
                        </div>
                    </Poptip>
            ...
            <div class="post-status-bar">
                ...
                <!--xx 保存于 date  保存后-->
                <!--xx 发布于 date  加载已发布-->
                <span>{{user.user_nickname}}</span>&nbsp;<span v-text="editorStatus"></span> <span
            ...
            <draggable :list="sidebarsOrder" class="dragArea">
                <transition-group type="transition" :name="'flip-list'">
                    <sidebar-panel v-for="sidebar in sidebarsOrder" :key="sidebar" class="postbox">
                        ...
                        <components :is="sidebar" :current-post="currentPost"
                                    @viewVersion="openVersionModel"></components>
                    </sidebar-panel>
                </transition-group>
            </draggable>
        </div>
        ...
        <version-modal ref="versionModal" :visible.sync="versionModel"
                       @restore="showVersionWarning = false"></version-modal>
                ...
                if (val) {
                    this.$Notice.warning({
                        ...
                        render: (h) => {
                            ...
                            return ['有一个自动保存的版本比如下显示的版本还更新', a]
                        }
                    })
                } else {
                    this.$Notice.close('version_notice')
