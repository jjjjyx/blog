...
class CommentDao extends db.BaseDao {
    ...
    async asyncInsert(data) {
        ...
        let temp = await this.asyncExec(sql, data);
    ...
    asyncGetCommentsByPosts(comment_post_id) {
        ...
        let sql = "select * from `myblog`.`j_comments` where comment_post_id = :comment_post_id ORDER BY comment_id,comment_karma DESC";
        return this.asyncExec(sql, {
            comment_post_id
        })
    }
