    ...
    <div class="icon-select-panel" transfer style="overflow: hidden;">
        ...
        <Tabs size="small" type="card" class="icon-select__body cm-tabs-style">
            <TabPane :label="group.label" v-for="(group, index) in groups" :key="index" class="icon-select__tab">
                <li v-for="icon in filterByTag(group.icons)" :key="icon.type" class="icon-select-icon__item"
            ...
            filterByTag (icons) {
                if (this.iconKey) {
                    return icons.filter((icon) => {
                        return find(icon.tag, (tag) => tag.indexOf(this.iconKey) >= 0) // icon.tag.indexOf(this.iconKey) >= 0
                    })
                } else {
                    return icons
                }
