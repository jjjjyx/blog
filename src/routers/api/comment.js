...
const {commentDao, userDao, sequelize} = require('../../models/index')
...
const getCommentById = [
    ...
    async function (req, res, next) {
        ...
        try {
            ...
            log.debug('加载对象 = %s 评论列表 共计 %d 条评论', parent, result.length)
...
const queryCommentKarma = `SELECT MAX(comment_karma) as max FROM j_comments WHERE comment_id = ?`

const comment = [
    // 如果频率次数过多给出验证码
    // 限制访问频率为 每分钟8
    // 验证 Referer
    rateLimiter.middleware({redis: client, key: ipAndRoute, rate: '8/m'}),
...
const writeUser = [
    ...
    // ip 限制访问频率为1 小时 5
    rateLimiter.middleware({redis: client, key: ipAndRoute, rate: '5/h'}),
...
router.route('/').get(getCommentById).post(comment)
