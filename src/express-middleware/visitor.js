...
log.trace('Load the visitor middleware and set the visitor information store as = req.cookies.%s', VISITOR_KEY)
...
const visitorMiddleware = function (req, res, next) {
    ...
    if (!jv && !/^__.{6,12}__$/.test(jv)) {
        ...
        opLog.info('Visitors access the system to generate cookies for %s = %s', VISITOR_KEY, vid)
